...
class ECARMetric(Metric):
    """Rate of correctly abstained predictions on gold-empty samples."""
    ...
    def compute(self, rows: list[EvaluationRow]) -> dict[str, float]:
        ...
        correct_abstained = sum(
            1
            for row in gold_empty_rows
            if row.parsed_output is not None and len(row.parsed_output.arguments) == 0
        )
        return {
            "ecar": safe_divide(
                correct_abstained,
